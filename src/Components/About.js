...
const About = () => {
  ...
  useEffect(() => {
    fetch("/Articles/about_me.md").then((r) => {
      r.text().then((t) => {
        setMd(t);
      });
    });
  ...
  const renderers = {
    code: ({ language, value }) => {
      return (
        <Flex direction="column" paddingTop="20px" paddingBottom="20px">
          <SyntaxHighlighter
            style={cb}
            language={language}
            children={value}
    ...
    paragraph: ({ children }) => {
      return <Text marginTop="5px" marginBottom="5px" children={children} />;
    },
    blockquote: ({ children }) => {
      return (
        <Center
    ...
    link: (p) => {
      ...
      return p.node.children[0].value == "iframe" ? (
        <Center>
          <iframe iframe width="1000px" height="700px" src={p.href} />
        </Center>
      ) : (
  ...
  return (
    <Flex direction="column" px={["5%", "10%", "15%", "20%"]}>
      <MySpacer height={10} />
